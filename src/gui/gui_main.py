...
class TargetDetectionGUI(tk.Frame):
    ...
    def create_widgets(self):
        ...
        # self.frame1 = tk.Frame(self.master, width=2560, height=1440, background="red")
        ...
        # self.canvas = DetectionCanvas(self.frame1, width=2560, height=1440)
        ...
        self.entry = ttk.Entry(self.frame2, textvariable=self.entry_text, width=200)  # 创建一个文本框，将StringVar变量绑定到它上面
        self.entry.grid(row=0, column=5)

        self.button = ttk.Button(self.frame2, text='change url', command=self.change_url)  # 创建一个按钮，点击时执行print_text函数
        self.button.grid(row=0, column=6)
        # 将两个框架添加到窗口中

        self.frame1.pack(side='top')
        self.frame2.pack(side='bottom')
    def polygon_initial(self, txt):
        self.polygon.clear_control_point()
        self.polygon.clear_point()
        now_id = self.config['url'][txt]
        points = self.config['id_points'][str(now_id)]
        for point in points:
    ...
    def change_url(self):
        if self.polygon_init:
            area_id = self.config['area']
            self.config['url']
        txt = self.entry_text.get()
        if txt in self.config['url']:
            if self.polygon_init:
                self.polygon_initial(txt=txt)
        else:
            self.polygon.clear_control_point()
            self.polygon.clear_point()
            now_id = self.config['url'][txt]
            ...
            self.polygon_controller = PolygonController(self.polygon,
                                                        self.canvas)
            self.polygon_init = True
        self.yolo.stream_loader = LoadStreams(txt, img_size=640)
        self.yolo.stream_loader.__iter__()
    ...
    def toggle_capture(self):
        ...
        if self.capturing:
            ...
            if not self.polygon_init:
                if self.yolo.url in self.config['url']:
                    self.polygon_initial(txt=self.yolo.url)
        else:
